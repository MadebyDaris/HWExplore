...
function extract_ir(f, argtypes::Type{<:Tuple})
    ...
    if ir === nothing
        error("[NexusV] Failed to extract IR for $(nameof(f)) with types $argtypes. " *
...
Extract the IR for `f` with `argtypes` and translate it into a NexusV Data-Flow Graph.
...
function extract_and_translate(f, argtypes::Type{<:Tuple}; name::Union{String,Nothing}=nothing)
    ...
    println("[NexusV] Synthesized '$(mod_name)' → $(length(graph.nodes)) DFG nodes, " *
