// hwx_top.sv
//
// Top-level wrapper that connects X-HEEP to the HWExplore coprocessor shell.
// This module owns the if_xif interface instance and performs the
// flat-to-interface translation required to bridge cvxif_hwx_shell
// to x_heep_system.
...
module hwx_top (
  ...
  logic [                31:0] sp_portB_rdata;

  hwx_scratchpad u_scratchpad (
      // Port A — AXI-Lite master (CPU writes config)
      .a_addr_i (axi_master_addr),
      .a_we_i   (axi_master_we),
      .a_wdata_i(axi_master_wdata),
      .a_rdata_o(axi_master_rdata),
      // Port B — stateful datapath
      .b_addr_i (sp_portB_addr),
      .b_we_i   (sp_portB_we),
      .b_wdata_i(sp_portB_wdata),
      .b_rdata_o(sp_portB_rdata)
  );

  // HWExplore Dispatcher Mux routes shell to datapath and back to shell.
  hwx_mux u_mux (
      .clk_i   (clk_i),
      .rst_ni  (rst_ni),
      ...
      // Stateful datapath interface
      .sp_addr_o (sp_portB_addr),
      .sp_wdata_o(sp_portB_wdata),
      .sp_we_o   (sp_portB_we),
      .sp_rdata_i(sp_portB_rdata)
  );
