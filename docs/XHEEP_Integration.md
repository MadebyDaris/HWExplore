...
```bash
source scripts/xheep_env.sh    # from the repo root, sets RISCV_XHEEP, COMPILER_PREFIX, PATH
cd hw/ext_xheep
make mcu-gen
```

This runs X-HEEP's own code generator against `general.hjson` and regenerates `core_v_mini_mcu_pkg.sv`, `cpu_subsystem.sv` (which is where the `cpu_features.cv_x_if` setting above actually takes effect), the OBI crossbar, and the FuseSoC `.vc` manifest that `build_hwx_sim.sh` consumes in the next step. **Any time you edit `general.hjson` or change `cpu_type`, rerun this**, it's the single most common cause of "I changed the config but nothing happened." `source scripts/xheep_env.sh` is not optional convenience, without the environment it sets, `mcu-gen` fails outright, and **it needs to be re-run in every new shell**, not just once, see [`Baremetal_and_System_Simulation.md` §5.1–5.3 and §5.7](Baremetal_and_System_Simulation.md#5-setting-up-from-scratch-every-blocker-and-its-fix) for exactly why and what each error looks like.
...
**Status: basic implementation done** (`emit_verilog(graph, path; share=Dict(OP_MUL => 1))`, `src/HWGen/ResourceSharing.jl`; results in [`Resource_Sharing_and_Macros.md`](Resource_Sharing_and_Macros.md)).

This exact spec is restated as a checkboxed task list (Phase 0) in [`DSE_Implementation_Plan.md`](DSE_Implementation_Plan.md), which also covers the PPA measurement methodology (standalone-module vs. whole-SoC synthesis, resolved there) and the Yosys/ABC tooling to actually measure the area/delay tradeoff this section describes.
