...
**Fastest path: run the setup script, in every new shell, before anything else below.**

```bash
source scripts/xheep_env.sh
```

This auto-detects a RISC-V toolchain (checking `~/.local/riscv-corev`, `~/riscv-corev`, `~/.riscv`, `/opt/riscv-corev`, in that order, install one there if none is found, it'll tell you how), sets `RISCV_XHEEP`/`RISCV_PREFIX`/`COMPILER_PREFIX` from it, and puts X-HEEP's own Python venv ahead of `PATH`. **It must be `source`d, not run**, it's setting environment variables in *your* shell, not a subshell. Re-run it in every new terminal; it doesn't persist across shells on its own (nothing does, that's how environment variables work, see §5.1 if that's surprising). Everything below explains what it's actually working around and how to do each piece by hand if you'd rather not rely on a script, or if it doesn't find a toolchain you already have installed.
...
The whole pipeline (SW build → HW unit tests → full-system build → full-system run) is one command from the repo root, after sourcing the env script:
...
To run just the full-system simulation step by step (same `source` first, every new shell):
...
`make mcu-gen` failing with a confusing error the first time you run it is normal, not a sign something is broken, X-HEEP's build auto-detects several things (Python environment, toolchain paths) that fail silently or misleadingly when the auto-detection guesses wrong. This section is the complete list of blockers hit getting a clean checkout to `EXIT SUCCESS`, in the order you'll hit them, because the error at each step rarely points at the actual cause. §5.1–5.4 are one-time-per-machine (toolchain install, config); §5.5–5.7 are things that come back, `scripts/xheep_env.sh` (§5.7) and reapplying `BOOT_ADDR` (§5.5) after every `mcu-gen` are the two you'll actually run repeatedly.
...
### 5.7 `make -C sw/platforms/xheep/tests` fails with `RISC-V toolchain not found: 'riscv-none-elf-gcc' is not on PATH`, in a *new* terminal, after everything already worked once

**Cause:** this isn't a regression, environment variables (`RISCV_PREFIX`, `PATH` additions) only last for the shell session they were set in. If you `export`ed them (or ran `source scripts/xheep_env.sh`) in one terminal, got a clean build, then opened a new terminal (or a new tab, or came back the next day) and ran `make` there without re-running that step, this is the expected result, not a sign the toolchain install was lost. (It's easy to mistake for "the install broke" the first few times this happens.)

**Fix:** `source scripts/xheep_env.sh` again, once per shell, every time. It's deliberately idempotent (safe to run repeatedly) and auto-detects the toolchain from the handful of locations §5.3's install command uses, so you don't need to remember the exact `RISCV_PREFIX` string. If you'd rather not depend on a script, add the three lines from §5.3 (`RISCV_XHEEP`, `COMPILER_PREFIX`, and the `.venv`'s `bin/` on `PATH`) to your shell's own startup file (`~/.bashrc`, `~/.zshrc`, ...) so every new shell has them automatically, the script exists for people who'd rather not touch their shell config, not because that's the only way.
